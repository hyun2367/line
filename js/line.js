$(function(){
    ...
    /* ==============================================
        언어상자 노출 스크립트
    ============================================== */ 
    // 설명 - 인풋, 화살표, 리스트 전체를 변수 지정 후 언어리스트의 a선택시, 클릭한 텍스트로 내용 변경
    // 클릭이 이루어진 a의 부모인 li에 on클래스 생성해주고, 나머지 형제들은 on 삭제
    // 언어 리스트 클릭시 보기/가리기
    // 인풋 내에 화살표 추가, 이를 클릭시 언어목록 보이면 숨기고, 숨겨져있으면 나타나도록 처리
    // 화살표 클락사 위를 향하는 화살표 클래스를 추가
    // 마우스가 선택한 언어박스에서 나갔을때 언어 리스트 가려주고 위를 향하는 화살표도 삭제

    // .text() - 텍스트를 불러오거나 바꿀 때 사용
    // .parent() - 부모선택
    // .toggle() - 선택한 요소가 보이면 숨기고, 숨겨져 있으면 나타나도록함
    ...
    // .scrollTop() - 브라우저의 스크롤바가 수직/수평으로 이동한 위치값을 불러오거나 변경할때 사용
    ...
    // swipe.js를 쓰려면, 순수 자바스크립트 방식(이전 방식)으로 문법을 써줘야한다.
    
    // swipe 영역 전체
    var $mainbanrSwipe = document.getElementById("mainbanner-swipe");
    // 인디케이터
    var $indicator = $("#mainbanner-swipe~.mainbanner-pagination>li>a") //인디케이터위치 지정

    var nowIdx = 0 /* Math.floor(Math.random()*4);*/
    var oldIdx = nowIdx;
    var intervalID = null;
    var noEventTime = 0; //이벤트가 없는 시간(초)를 체크하는 변수
    var $btnAuto = $("#mainbanner-swipe~.btn_auto"); // 시작, 정지버튼
    $btnAuto.data("state","on");
    // 버튼에 내부 데이터 설정 - 재생중인 상태와 정지상태
    // null은 변수에 지정된 데이터를 지우고자 할 때 사용
   ...
   // Swipe 플러그인 사용 문법
    ...
    $btnAuto.on("click",function(evt){
        ...
        if($(this).data("state")=="on"){
            ...
            stateVal="on" // 내부 데이터의 값을 on 으로 한다.
            autoPlay(); //자동재생 함수 호출
        }
